...
# The input is current chessboard.
    def go(self, chessboard):
        ...
        if self.candidate_list:
            ...
            if turn<=self.cut:
                ...
                for s in list(self.Vs.keys()):
                    if np.count_nonzero(np.fromstring(s,dtype=np.int))<turn:
                        del self.Vs[s]
                        del self.Ns[s]
                        del self.Es[s]
                for s,a in list(self.Nsa.keys()):
                    if np.count_nonzero(np.fromstring(s,dtype=np.int))<turn:
                        del self.Nsa[(s,a)]
                        del self.Qsa[(s,a)]
    ...
    def alpha_beta(self, board, cutoff=True):
        ...
        def max_value(board, color, alpha, beta, depth):
            if depth==0: return eval(board*self.color)
    ...
    def _search(self, canonicalBoard):
        ...
        for a in moves:
            if (s, a) in self.Qsa:
                ...
                u = self.cpuct * np.sqrt(self.Ns[s] + 1e-8)  
            if u > cur_best:
                cur_best = u
                best_act = a
